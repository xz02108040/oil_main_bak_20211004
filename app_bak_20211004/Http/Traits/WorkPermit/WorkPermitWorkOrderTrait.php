...
trait WorkPermitWorkOrderTrait
{
    ...
    public function createWorkPermitWorkOrder($data,$mod_user = 1)
    {
        ...
        $INS->b_car_memo                = isset($data->b_car_memo)? $data->b_car_memo : '';
    ...
    public function setWorkPermitWorkOrder($id,$data,$mod_user = 1,$hrefMain = '')
    {
        ...
        //廠區->施工地點
        if(isset($data->b_factory_b_id) && $data->b_factory_b_id && $data->b_factory_b_id !== $UPD->b_factory_b_id)
        {
            $isUp++;
            $UPD->b_factory_b_id = $data->b_factory_b_id;
        }
        //廠區->door
        ...
        {
            $isUp++;
            $UPD->eta_time1 = $UPD->eta_time; // 若修改則將原預計工作結束時間存到 eta_time1
     ...
     * @return array
     */
    public function getApiWorkPermitWorkOrderByProcess($process_type,$dept_id,$title_id = 0,$store_id = 0)
    {
        $dept1 = $dept2 = $dept3 = $dept4 = $dept5 = 0;
        if($process_type == 5 && !in_array($title_id,[2,3,4])) return 0; //轄區主簽者階段: 轄區主管,轄區主簽者,廠區主簽者可簽
        ...
        if($process_type == 5)
        {
            if(in_array($title_id,[2,3]))  //轄區主管,轄區主簽者需指定部門
            {
                //$dept5    = $dept_id; //dept5 為上級部門
                $dept1    = $dept_id;
            }
        ...
        if($title_id == 4 && $store_id)
        {
            $data->whereIn('view_wp_work.b_factory_id',[$store_id]);
        }
        ...
        if($dept2)
        {
            $data = $data->where('view_wp_work.be_dept_id2',$dept2);
        }
        if($dept3)
        {
            $data = $data->where('view_wp_work.be_dept_id3',$dept3);
        }
        if($dept4)
        {
            $data = $data->where('view_wp_work.be_dept_id4',$dept4);
        }
        if($dept5)
        {
            $data = $data->where('view_wp_work.be_dept_id5',$dept5);
        }
     ...
     * @return array
     */
    public function getApiWorkPermitWorkOrderList($isSupply ,$aproc = ['A'],$wpSearch = [0,0,'','',0],$storeSearch = [0,0,0],$depSearch = [0,0,0,0,0,0],$dateSearch=['','',''],$appSearch = ['N',0],$isGroupBy = 'N',$bc_id='')
    {
        $ret = array();
        ...
        //晚班查詢跨日時間限制
        $work_max_time = sys_param::getParam('WP_WORK_YESTERDAY_MAX_TIME');
        //成員身分證
        $bc_id = isset($bc_id) ? $bc_id : '';
        ...
        if($bc_id)
        {
            $data = $data->join('wp_work_worker as ww','ww.wp_work_id','=','wp_work.id');
            $data = $data->join('view_user as vu','ww.user_id','=','vu.b_cust_id');
            $data = $data->where('vu.bc_id',$bc_id);
        }
        //承攬商ID
        if($b_supply_id)
        {
        ...
        if($isNow == 'Y')
        {
            ...
            if($sdate && !$edate)
            {
                if(time() > strtotime($today.$work_max_time))
                {
        ...
        if($count)
        {
            ...
            foreach ($data as $k => $v)
            {
                if($isGroupBy == 'Y')
                {
                    ...
                    $data[$k]['eta_time1']          = !is_null($v->eta_time1)?substr($v->eta_time1,0,16) : '';
                    
                    // 延長工時審核人
                    $data[$k]['extension_charge_user_name'] = '';
                    if (isset($v->wp_work_rp_extension_id) && !empty($v->wp_work_rp_extension_id)) {
                        $res = User::join('wp_work_rp_extension', 'b_cust.id', 'wp_work_rp_extension.charge_user2')
                        ->where('wp_work_rp_extension.id', $v->wp_work_rp_extension_id)
                        ->select('b_cust.name')->first();
                        $data[$k]['extension_charge_user_name'] = isset($res) ? $res->name : '';
                    }
                    ...
                    $data[$k]['start_memo'] = Lang::get('sys_workpermit.'.$no_start_memo);   //啟動失敗原因
                    ...
                    $data[$k]['isRollCall'] = ($isSupply && $v->aproc == 'R' && wp_work_check::isExist($v->id,2))? 'Y' : 'N';   //是否可以局限空間
                    $data[$k]['isPatrol']   = (!$isSupply && $v->aproc == 'R')? 'Y' : 'N';   //是否巡邏
                    //施工進度
                    $processData                    = wp_work_list::getNowProcessStatus($v->id);
                    $data[$k]['stime1']             = isset($processData['work_stime'])? $processData['work_stime'] : '';
                    $data[$k]['etime1']             = isset($processData['work_etime'])? $processData['work_etime'] : '';
//                    dd($processData);
                    ...
                    $data[$k]['process_lock']           = isset($processData['process_lock'])? $processData['process_lock'] : '';
                    //2021-04-02
                    ...
                    $data[$k]['isStop']                 = (in_array($v->aproc,['F','C']))? 'N' : $data[$k]['isStop'];   //
                    //是否可以收工
                    $data[$k]['isOffWork']              = ($isSupply && in_array($data[$k]['now_process_id'],[7,24]))? 'Y' : 'N';   //是否可以暫停

                    if($isApp == 'Y')
                    {
                        if(in_array($v->aproc,['W','P','R','O']))
                        {
                            list($isOp) = HTTCLib::isTargetList($v->id,$user);
                            $data[$k]['isStop']     = $isOp ? $data[$k]['isStop'] : 'N';
                        }
...
//                    dd($processData);
    ...
    public function getWorkPermitWorkOrderCheckFile($work_id)
    {
        ...
        where('f.isClose','N')->select('f.id','k.name as kind','f.name','f.path','f.router','f.wp_check_id1','f.wp_check_id2','f.wp_check_id3')->
        ...
        if($data->count())
        {
            ...
            foreach ($data as $val)
            {
                ...
                $tmp['kind'] = $val->kind;
                $tmp['name'] = $val->name;
                $tmp['router'] = $val->router;
                $tmp['wp_check_id1'] = $val->wp_check_id1;
                $tmp['wp_check_id2'] = $val->wp_check_id2;
                $tmp['wp_check_id3'] = $val->wp_check_id3;
    ...
    {
        $ret   = [];
        $data  = view_wp_work::where('aproc','R')->where('isClose','N')->select('id');
    ...
    {
        $ret   = [];
        $data  = view_wp_work::whereIn('aproc',['C','F'])->select('id','etime1')->where('isClose','N');
        $needRegularAmt = $data->count();
        if($needRegularAmt)
        {
            $data = $data->get();
            foreach ($data as $val)
            {
                //2020-02-03 一個小時才通知
                $nowTime = time();
                $endTime = strlen($val->etime1)? strtotime($val->etime1) : $nowTime;
                if(($nowTime - $endTime) >= 3600)
                {
                    //如果場內依舊有人
                    $inMenAry = wp_work_worker::checkWorkerNotInWork($val->id);
                    if(count($inMenAry))
                    {
                        $tmp = [];
                        $tmp['id']  = $val->id;
     ...
     * 取得 當日工作許可證 已經收工完成，但是尚未離場
     */
    public function getWorkPermitWorkOrderCheckRegular3()
    {
        $ret   = [];
        $data  = view_wp_work::join('wp_work_list as l','l.wp_work_id','=','view_wp_work.id')->
            join('wp_work_process as p','p.id','=','l.wp_work_process_id')->
            where('p.wp_permit_process_id',24)->where('p.isClose','N')->
            select('view_wp_work.id','p.stime')->where('view_wp_work.isClose','N');
        $needRegularAmt = $data->count();
        if($needRegularAmt)
        {
            $data = $data->get();
            foreach ($data as $val)
            {
                //2020-04-28
                $nowTime = time();
                $endTime = strlen($val->stime)? strtotime($val->stime) : $nowTime;
                if(($nowTime - $endTime) >= 3600)
                {
                    // 找沒有掛上工單ＩＤ
                    $inMenAry = wp_work_worker::checkWorkerNotInWork($val->id);
     ...
     * @return array
     */
    public function getApiWorkPermitInReWorkProcess()
    {
        $ret = [];
        //
        $data  = view_wp_work::join('wp_work_list as l','l.wp_work_id','=','view_wp_work.id')->
        join('wp_work_process as p','p.id','=','l.wp_work_process_id')->
        whereIn('p.wp_permit_process_id',[24,25])->where('p.isClose','N')->
        select('view_wp_work.id','view_wp_work.eta_time')->where('view_wp_work.isClose','N');
        $needRegularAmt = $data->count();
        if($needRegularAmt)
        {
            $data = $data->get();
            ...
            {
                $nowTime = time();
                $endTime = strlen($val->eta_time)? strtotime($val->eta_time) : $nowTime;
                //2020-04-28
                if($endTime < $nowTime)
                {
                    $ret[] = $val->id;
