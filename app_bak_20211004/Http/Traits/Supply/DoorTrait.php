...
trait DoorTrait
{
    ...
    public function createCoursePass($uid = 0)
    {
        ...
        //STEP1 :
        // 教育訓練通過名單，期限未過期
        $traningData    = DB::table('view_supply_etraning');
        if($uid)
        {
            $traningData = $traningData->where('b_cust_id',$uid);
        }
        $traningList    = $traningData->get()->toArray();
        //STEP2 : 轉換 人員->通過課程 之陣列
        $traningAry = $traningOKAry   = [];
        foreach ($traningList as $val)
        {
            $b_cust_id      = $val->b_cust_id;      //成員
            $et_course_id   = $val->et_course_id;   //通過課程
            $edate          = $val->valid_date;

            $traningAry[$b_cust_id][$et_course_id]['cid'] = $et_course_id;
            $traningAry[$b_cust_id][$et_course_id]['edate'] = $edate;
        }
//        dd($traningAry);
        //STEP3 : 教育訓練 資格通過
        foreach ($traningAry as $uid => $course)
        {
            //$project_id     = view_door_supply_member::getProjectID($uid); //目前參與工程案件
            //$courseAry      = e_project_c::getSelect($project_id,1);   //該工程案件指定 課程<門禁有關>
            $courseParam    = sys_param::getParam('DOOR_COURSE_ID',1);   //該工程案件指定 課程<門禁有關>
            $courseAry      = explode(',',$courseParam);
            $allowCourseCnt = 0; // 已經上課<指定課程>數量
            $MaxCourseCnt   = count($courseAry); //該工程案件指定 課程 數量
            //dd([$project_id,$course,$courseAry,$MaxCourseCnt]);

            foreach ($course as $cval)
            {
                $cid   = isset($cval['cid'])? $cval['cid'] : 0;
                $edate = isset($cval['edate'])? $cval['edate'] : $today;
                if(in_array($cid,$courseAry))
                {
                    $allowCourseCnt++;
                }
            }

            if($MaxCourseCnt <= $allowCourseCnt) $traningOKAry[$uid] = $edate;
        }
        //關閉全部 教育訓練 資格通過的人員紀錄
        LogLib::closeTodayCoursePassLog();
        //寫入今日 教育訓練 資格通過的人員紀錄
        if(count($traningOKAry))
        {
            foreach ($traningOKAry as $uid => $edate)
            {
                if(LogLib::putCoursePassLog($uid,$edate))
                {
                    $sucAmt++;
                } else {
                    $errAmt++;
        ...
        $retStr = Lang::get('sys_base.base_10127',['suc'=>$sucAmt,'err'=>$errAmt,'json'=>json_encode($traningOKAry)]);
        return [count($traningOKAry),$retStr];
    }
    ...
    public function createDoorInoutRecord($bcid,$b_factory_id,$b_factory_d_id,$mode,$time,$img = '',$isDoorRecord = 'Y')
    {
        ...
        //1-4. 判斷 未來日期
        $isOver  = ($isOnline == 'Y' && (strtotime($this->time) > (time() + $door_max_over_time_limit)))? 'Y' : 'N';
...
//        dd($b_factory_id,$bcid,$carAry);
        ...
        if($isUser && !$isExist)
        {
            ...
            if(isset($isBlackListAry->error) )
            {
                ...
                $door_result_name = Lang::get('sys_base.base_30151');
                $strcode          = ($isUser)? 'base_30102' : 'base_30106';

                $name           = Lang::get('sys_base.base_30165');
                $door_memo1     = Lang::get('sys_base.base_30100',['door_result'=>$door_result_name,'door_stamp'=>$time]);
                $door_memo2     = Lang::get('sys_base.'.$strcode ,['name'=>$name,'supply'=>$b_supply,'project'=>$project]);
                $door_memo3     = Lang::get('sys_base.base_30103',['door_type'=>$door_result_name]);
                ...
                //2019-11-08 配合昱俊
                $door_violation2    = $violation_memo ? Lang::get('sys_base.base_30117',['name'=>$name,'door_type'=>$door_result_name,'memo'=>$violation_memo]) : '';

                $resultAry = [];
                $resultAry[] = ['',''];
                $resultAry[] = [Lang::get('sys_base.base_30116'),$door_violation2];

                return [-1,'N',$door_memo,$isOnline,0,$resultAry];
            }
        }
        ...
        if($isExist > 0)
        {
            if($isOnline == 'N' || $isOnline2 == 'N') {
                ...
                if($logid)
                {
                    return [$logid,$door_result,$door_memo,$isOnline,$door_type,$door_data];
                } elseif($isExist > 0) {
                    $door_result    = 'Y'; //可以進入
                    $door_memo      = '';
                    $door_type      = 0;
            ...
            if($last_log_id)
            {
                ...
                $door_result_name   = isset($doorResultAry[$last_result])? $doorResultAry[$last_result] : Lang::get('sys_base.base_30166');
                if($last_result == 'N')
                {
...
//                $door_data[] = [Lang::get('sys_base.base_30116'),$violation_memo];
                }
...
//            dd([$isExist,$door_result,$mode,$door_type,$isOnline,$lastDoorType]);
            ...
            if($isExist == -1)
            {
                ...
                if($isSupply && $isUser && $isOnline == 'Y' && $door_result == 'Y' && $jobkind !=  4)
                {
                    /*
                     * 門禁許可檢查規則
                     * 1: 不檢查 工安＆工負
...
//                        dd($door_rule,$work_aproc,$isRoot);
                            ...
                            //身份：工地負責人（工程案件）
//                        if($jobkind == 2) $jobkindName = Lang::get('sys_base.base_40231');
                            //身份：安衛人員（工程案件）
//                        if($jobkind == 3) $jobkindName = Lang::get('sys_base.base_40232');
...
//                            }
                        ...
                        elseif($door_type == 2)
                        {
                            if($door_rule == 2 && in_array($jobkind,[1,3]))
                            {
                            ...
                            if($isRoot && $door_result == 'Y')
                            {
                                ...
                                elseif (count($adUserAry))
                                {
                                    if($work_id)
                                    {
                                        ...
                                        $isWorkerExist = rept_doorinout_t::isExist($b_factory_id,$b_factory_d_id,$door_date,0,1,$adUserAry,0,$e_project_id);
                                    }
...
//            dd($isUser , $door_type , $work_id , $isLevel, $door_result,$violation_memo);
            ...
            //dd([$retAry,$errCode,$isOnline,$isOver]);
            //4-2. 寫入Log
            ...
            if($logid && $door_result == 'Y')
            {
                ...
                if($isUser && $door_type == 2 && $work_id)
                {
                    $this->setWrokPermitWorkerMenOut($work_id,$bid);
                }

                //4-5. 啟動 工作許可證 補人之成員
                if($isUser && $door_type == 1 && $work_id && $isAdd)
                {
                ...
                if($isUser && $door_type == 2 && $work_id && $isLevel)//&& $isLevel
                {
        ...
        return [$logid,$door_result,$door_memo,$isOnline,$door_type,$resultAry];
    }
    ...
    public function getWhiteList($dataKind = 1, $type = 1, $img_at = 0)
    {
        ...
        if(in_array($dataKind,[1,2]))
        {
            ...
            select('name','b_cust_id','bc_id','head_img_at','head_img','rfid_code','error')->get()->toArray();
            ...
            select('name','b_cust_id','bc_id','head_img_at','head_img','rfid_code')->get()->toArray();
    ...
    public function getCarWhiteList($b_factory_id = 0)
    {
        ...
        if(count($carAry)) {
            foreach ($carAry as $key => $val) {
                //用戶名單
                $supplyAry[$no] = [];
                $supplyAry[$no]['id'] = $val->b_supply_id;
                $supplyAry[$no]['name'] = $val->supply;
                //車牌名單
                $whitelistAry[$no] = [];
                ...
                } elseif(in_array($val->b_car_id,$carViolationAry)) {
                    //違規
                    $whitelistAry[$no]['rule'] = 'N';
