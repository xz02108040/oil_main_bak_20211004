...
class EngineeringMemberIdentityController extends Controller
{
    use EngineeringMemberTrait,EngineeringTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | EngineeringMemberIdentityController
    |--------------------------------------------------------------------------
    |
    | 工程案件之承攬商成員之工程身分 維護
    |
    */
    ...
    public function __construct(Request $request)
    {
        ...
        $this->hrefMain         = 'engineeringmemberidentity';
        $this->langText         = 'sys_engineering';

        $this->hrefMainDetail   = 'engineeringmemberidentity/';
        $this->hrefMainNew      = 'new_engineeringmemberidentity/';
        $this->routerPost       = 'postEngineeringmemberidentity';
    ...
    public function index(Request $request)
    {
        ...
        }elseif(!$uid && !(is_numeric($uid) && $uid > 0))
        {
            //查無工程案件之成員
            $msg = Lang::get($this->langText.'.engineering_1079');
            return \Redirect::back()->withErrors($msg);
        } else {
            $param = 'pid='.$request->pid.'&uid='.$request->uid;
        }
        //view元件參數
        ...
        $listAry = $this->getApiEngineeringMemberIdentityList($pid,$uid);
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.engineering_111')]; //專業證照
        $heads[] = ['title'=>Lang::get($this->langText.'.engineering_111')]; //專業證照
