...
{
    use EngineeringCourseTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | EngineeringCourseController
    |--------------------------------------------------------------------------
    |
    | 工程案件所需 教育訓練 維護
    |
    */
    ...
    public function __construct()
    {
        ...
        $this->hrefMain         = 'engineeringcourse';
        $this->langText         = 'sys_engineering';

        $this->hrefMainDetail   = 'engineeringcourse/';
        $this->hrefMainNew      = 'new_engineeringcourse/';
        $this->routerPost       = 'postEngineeringcourse';
    ...
    public function index(Request $request)
    {
        ...
        $supply_id= e_project::getSupply($pid);
        ...
        $listAry = $this->getApiEngineeringCourseList($pid);
        ...
        $form->addHtml(HtmlLib::Color(Lang::get($this->langText.'.engineering_1034'),'red',1)); //說明
        $form->addHr();
        //輸出
        $out .= $form->output(1);
        //table
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.engineering_161')];//目前通過人數
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                $no++;
                $id           = $value->id;
                $name1        = $value->e_project; //
                $name2        = $value->name; //
                $name3        = HtmlLib::btn('etraningsupplymember?sid2='.SHCSLib::encode($supply_id),$value->pass_amt,'3','','','','_blank'); //
    ...
    public function show(Request $request,$urlid)
    {
        ...
        if(!isset($getData->id))
        {
            ...
            //資料明細
            $A1         = $getData->e_project; //
            $A2         = $getData->et_course_id; //
        ...
        //證照
        $html = $form->select('et_course_id',$selectAry,$A2);
    ...
    {
        //資料不齊全
        if( !$request->has('agreeY') || !$request->id || !$request->pid || !$request->et_course_id )
        {
            return \Redirect::back()
                ->withErrors(Lang::get('sys_base.base_10103'))
                ->withInput();
        }
        elseif(e_project_c::isExist(SHCSLib::decode($request->pid),$request->et_course_id,SHCSLib::decode($request->id)))
        {
            return \Redirect::back()
        ...
        $upAry['et_course_id']      = is_numeric($request->et_course_id) ? $request->et_course_id : 0;
        $upAry['isClose']           = ($request->isClose == 'Y')? 'Y' : 'N';

        //新增
        if($isNew)
        {
            $ret = $this->createEngineeringCourse($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setEngineeringCourse($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'e_project_c',$id);
    ...
    public function create($urlid)
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.engineering_1'),1);
        //證照
        $html = $form->select('et_course_id',$selectAry,1);
        $form->add('nameT3', $html,Lang::get($this->langText.'.engineering_42'),1);
