...
class ReportApiController extends Controller
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        //存放資料夾位置
        $this->receivePath        = config('mycfg.api_receive_path3','/API/REPT_RECEIVE/').date('Y/m/d/H/i/s/');//接收資料夾
        $this->replyPath          = config('mycfg.api_reply_path3','/API/REPT_REPLY/').date('Y/m/d/H/i/s/');//傳送資料夾
    }
...
//            Storage::put($this->receivePath.Uuid::generate(1), $jsonData);
    ...
    public function resolver($jsonData)
    {
        ...
        $className = 'App\\API\\Report\\' . $funcode;
        $classPath = app_path().'/API/Report/' . $funcode . '.php';
        //3. 確認「格式代碼」 是否存在
