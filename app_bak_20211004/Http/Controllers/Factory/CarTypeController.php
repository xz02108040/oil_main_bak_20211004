...
use App\Http\Traits\Factory\CarTypeTrait;
...
class CarTypeController extends Controller
{
    ...
    */
    ...
    public function __construct(Request $request)
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'cartype';
        $this->langText         = 'sys_supply';

        $this->hrefMainDetail   = 'cartype/';
        $this->hrefMainNew      = 'new_cartype';
        $this->routerPost       = 'postCartype';
...
//        $btnBack  = $this->pageBackBtn;
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiCarTypeList();
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.supply_94')]; //車輛動力
    ...
    public function show(Request $request,$urlid)
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.supply_2'),1);
        //動力
        $html = $form->select('oil_kind',$oilkindAry,$A3);
        $form->add('nameT3', $html,Lang::get($this->langText.'.supply_94'));
        //排序
        $html = $form->text('show_order',$A2);
        $form->add('nameT3', $html,Lang::get($this->langText.'.supply_11'));
        //停用
        $html = $form->checkbox('isClose','Y',$A99);
        $form->add('isCloseT',$html,Lang::get('sys_base.base_10613'));
        if($A99)
        {
            $html = $A97;
    ...
    public function post(Request $request)
    {
        ...
        //重複名稱
        elseif(b_car_type::isExist($request->name,SHCSLib::decode($request->id)))
        {
            return \Redirect::back()
                ->withErrors(Lang::get('sys_supply.supply_1031'))
                ->withInput();
        }
        else {
            $this->getBcustParam();
        ...
        $upAry['oil_kind']          = is_numeric($request->oil_kind) ? $request->oil_kind : 1;
        ...
        if($isNew)
        {
            $ret = $this->createCarType($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setCarType($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'b_car_type',$id);
    ...
    public function create()
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.supply_2'),1);
        //動力
        $html = $form->select('oil_kind',$oilkindAry,1);
        $form->add('nameT3', $html,Lang::get($this->langText.'.supply_94'));
        //排序
