...
class Rept46Controller extends Controller
{
    use SessTraits;
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'report_46';

        $this->pageTitleMain    = '舊系統原始案件資料';//大標題
...
//        if(!$aid || !$bid)
        {
            $SQL = "SELECT Project_no,Project_name,CONVERT(VARCHAR(10),sdate,120) sdate,CONVERT(VARCHAR(10),edate,120) edate,
						   tax_num,eng_pic_id1,eng_pic1,eng_pic_id,
						   eng_pic,guard_id1,guard1,A.guard_id,
						   A.guard,spotter,A.tel,unit,location,
						   finish,finish_date,B.name FROM guard.dbo.View_eng A 
						   JOIN guard.dbo.company B ON A.tax_num=b.company_id
					WHERE edate>dateadd(month,-2,getdate()) AND (finish IS NULL OR finish='A')
					AND Project_no NOT IN ('0123456789','12345')
					AND Project_name NOT LIKE '%溪州%' AND Project_name NOT LIKE '%溪洲%'
					";	
