...
class Rept2Controller extends Controller
{
    use SessTraits;
    /*
    |--------------------------------------------------------------------------
...
//        if(!$aid || !$bid)
        {
            ...
            $SQL = "SELECT A.id,A.rfid_code,B.name AS USERNAME,A.job_kind,B.account,A.door_date,
					convert(varchar(19),A.door_stamp,120) AS door_stamp, A.door_type,A.door_memo, A.door_result,E.name AS factory_d,
					D.sub_name,D.project_no ,D.name AS project_name,A.img_path, G.supply_in_count
					FROM dbo.log_door_inout A 
					JOIN dbo.b_cust B ON A.b_cust_id=B.id
					JOIN (SELECT DISTINCT A.id,A.project_no,A.name,B.b_supply_id,C.sub_name,B.b_cust_id FROM dbo.e_project A JOIN dbo.b_supply_member B
                    ...
                    JOIN (SELECT A.b_supply_id, A.door_date, COUNT(DISTINCT(A.b_cust_id)) AS supply_in_count FROM dbo.log_door_inout A WHERE A.door_type = 1 GROUP BY A.b_supply_id, A.door_date) G ON G.b_supply_id=A.b_supply_id AND G.door_date=A.door_date -- 承攬商人員入場人數
					WHERE :account IN (B.account,'') AND :username IN (B.name,'') AND :factory IN (A.b_factory_id,'')
					AND :factory_d_id IN (A.b_factory_d_id,'') AND A.door_date BETWEEN :sdate AND :edate
					AND A.err_code=0 AND A.door_type IN (1,2) $query
					";
					

			//$listAry = DB::select($SQL);
			$listAry = DB::select($SQL,['sdate'=>$sdate,'edate'=>$edate,'tax_num'=>$did,'project_no'=>$eid,'supply_id'=>$bid,'account'=>$gid,'username'=>$fid,'factory'=>$aid,'factory_d_id'=>$cid]);
            //Excel
