...
class Rept43Controller extends Controller
{
    use SessTraits;
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'report_43';

        $this->pageTitleMain    = '門禁卡片遺失清單';//大標題
...
//        if(!$aid || !$bid)
        {
            $SQL = "SELECT A.rfid_code,
                    (SELECT TOP 1 C.sub_name FROM b_rfid_a B JOIN b_supply C ON B.b_supply_id=C.id WHERE A.id=b.b_rfid_id ORDER BY B.close_stamp DESC)  AS sub_name,
                    (SELECT TOP 1 D.name FROM b_rfid_a B JOIN b_cust D ON B.b_cust_id=D.id WHERE A.id=b.b_rfid_id ORDER BY B.close_stamp DESC)  AS 人員,
                    CONVERT(VARCHAR(10),A.close_stamp,120) AS close_stamp
                    FROM dbo.b_rfid A 
					WHERE A.rfid_invalid_type='2' AND CONVERT(VARCHAR(10),A.close_stamp,120) BETWEEN '".$sdate."' AND '".$edate."'";
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                ...
                $rept2           = $value->sub_name;
                $rept3  		 = $value->人員;
				$rept4  		 = $value->close_stamp;
