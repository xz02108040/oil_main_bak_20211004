...
class Rept59Controller extends Controller
{
    use SessTraits;
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'report_59';

        $this->pageTitleMain    = '承攬商車輛累計工時報表';//大標題
...
//        if(!$aid || !$bid)

        {
            ...
            if($bid != 0){
                ...
                $SQL = "SELECT B.name  ,A.car_no, A.job_kind, E.name AS e_name,
					...
					FROM dbo.log_car_hours A
					JOIN dbo.b_supply B ON A.b_supply_id=B.id
                    JOIN dbo.e_project E ON A.e_project_id = E.id
					WHERE A.b_supply_id = '".$bid."' $query AND :car_no in (A.car_no,'')
					order by door_enter_time";

                $SQL2 = "SELECT convert(varchar,SUM(second)/3600)+'小時'+convert(varchar,SUM(second)%3600/60)+'分鐘' AS total_hours
					FROM dbo.log_car_hours A
					JOIN dbo.b_supply B ON A.b_supply_id=B.id
                    JOIN dbo.e_project E ON A.e_project_id = E.id
					WHERE A.b_supply_id = '".$bid."' $query AND :car_no in (A.car_no,'')
                    GROUP BY A.b_supply_id

					";
                $listAry = DB::select($SQL, ['car_no'=>$fid]);
                $listAry_total = DB::select($SQL2,  ['car_no'=>$fid]);
                if(!$listAry_total){
                    $total_hours = '0小時0分鐘';
                }else{
                    $total_hours =  $listAry_total[0]->total_hours;
                }
            }else{
                $listAry = [];
            }
            ...
            {
                $excelReport = [];
                $excelReport[] = ['承攬商','工程案件','車牌','車類型','進場時間','出場時間','總工時'];
        ...
        $form->addRowCnt($html);
        //至少一個搜尋條件
        $html = HtmlLib::Color('說明：請選擇一個承商','red',1);
        $form->addRow($html);
        $form->addHr();
        ...
        $totalRow = '<p>';
        $totalRow .= '總工時 : ' . HtmlLib::Color($total_hours,'red',1);;
        $totalRow .= '</p>';
