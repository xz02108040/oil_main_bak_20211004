...
class Rept19Controller extends Controller
{
    use SessTraits;
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'report_19';

        $this->pageTitleMain    = '違規事項排行板';//大標題
    ...
    public function index(Request $request)
    {
		...
		$rank      =array('10'=>'10','50'=>'50','100'=>'100');
        ...
        {
            $sdate = $edate = $months = $years = $aid = $hid = '';
        ...
        if(!$aid)
        {
            $aid = Session::get($this->hrefMain.'.search.aid',50);
        } else {
...
//        if(!$aid || !$bid)
        {
            ...
            if($hid=='1')
			{
				$query .= "A.apply_date BETWEEN '".$sdate."' AND '".$edate."'";
			}
			else if($hid=='2')
			{
				$query .= "convert(varchar(7),A.apply_date,120)= '".$months."'";
			}else if($hid='3')
			{
				$query .= "DATEPART(YEAR,A.apply_date)='".$years."'";
			}
            $SQL = "SELECT TOP ".$aid." --設定排名資料
					A.e_violation_id, A.violation_record1,COUNT(*) 筆數
					FROM e_violation_contractor A
					--WHERE DATEPART(MONTH,A.apply_date)=1      --(月份)
					--AND DATEPART(YEAR,A.apply_date)=2019     --(年度)
                    WHERE $query
					GROUP BY A.e_violation_id, A.violation_record1
					ORDER BY 筆數 DESC
					";
            ...
            {
                $excelReport = [];
                $excelReport[] = ['編號','違規事項','筆數'];
                foreach ($listAry as $value)
                {
                    $tmp    = [];
					$tmp[]  = $value->e_violation_id;
                ...
                return Excel::download(new ExcelExport(), '違規事項排行板_'.date('Ymdhis').'.xlsx');
            }
        }
        //-------------------------------------------//
        //  View -> Gird
        ...
        if(count($listAry))
        {
            ...
            {
                $no++;
                $rept1           = $value->e_violation_id;
                $rept2           = $value->violation_record1;
                $rept3           = $value->筆數;
