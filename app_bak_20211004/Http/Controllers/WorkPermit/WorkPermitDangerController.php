...
class WorkPermitDangerController extends Controller
{
    use WorkPermitDangerTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | WorkPermitDangerController
    |--------------------------------------------------------------------------
    |
    | 工作許可證 危險等級維護
    |
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'workpermitdanger';
        $this->hrefKind         = 'workpermitkind';
        $this->hrefItem         = 'workpermitworkitem';
        $this->langText         = 'sys_workpermit';

        $this->hrefMainDetail   = 'workpermitdanger/';
        $this->hrefMainNew      = 'new_workpermitdanger/';
        $this->routerPost       = 'postWorkpermitdanger';
    ...
    public function index(Request $request)
    {
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiWorkPermitDangerList();
    ...
    public function show(Request $request,$urlid)
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_44'),1);
        //種類
        $html = $A2.$form->hidden('workitem_id',$A4);
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_108'),1);
        //種類
        $html = $form->ckeditor('context',$A5,8);
    ...
    {
        //資料不齊全
        if( !$request->has('agreeY') || !$request->id || !$request->name  || !$request->context )
        {
        ...
        if($isNew)
        {
            $ret = $this->createWorkPermitDanger($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setWorkPermitDanger($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'wp_permit_danger',$id);
                //更新工作許可證版本
                wp_permit::updateAt();
