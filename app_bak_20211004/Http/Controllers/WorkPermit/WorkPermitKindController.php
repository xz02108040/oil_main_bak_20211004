...
class WorkPermitKindController extends Controller
{
    use WorkPermitKindTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | WorkPermitKindController
    |--------------------------------------------------------------------------
    |
    | 工作許可證 種類維護
    |
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'workpermitkind';
        $this->hrefDanger       = 'workpermitworkitem';
        $this->langText         = 'sys_workpermit';

        $this->hrefMainDetail   = 'workpermitkind/';
        $this->hrefMainNew      = 'new_workpermitkind';
        $this->routerPost       = 'postWorkpermitkind';
...
//        $btnBack  = $this->pageBackBtn;
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiWorkPermitKindList();
        ...
        //標題
        $heads[] = ['title'=>'NO'];
        $heads[] = ['title'=>Lang::get($this->langText.'.permit_2')]; //名稱
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                ...
                $btn2         = HtmlLib::btn(SHCSLib::url($this->hrefDanger,'','kid='.SHCSLib::encode($id)),Lang::get('sys_btn.btn_30'),4); //按鈕
    ...
    public function post(Request $request)
    {
        ...
        if($isNew)
        {
            $ret = $this->createWorkPermitKind($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setWorkPermitKind($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'wp_permit_kind',$id);
