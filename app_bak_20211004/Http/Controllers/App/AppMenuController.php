...
{
    use AppMenuTrait,AppMenuAuthTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | AppMenu Controller
    |--------------------------------------------------------------------------
    ...
    */
    ...
    public function __construct(Request $request)
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'app_menu';
        $this->hrefMainSelect   = 'app_menu_select';
        $this->langText         = 'sys_base';

        $this->hrefMainDetail   = 'app_menu/';
        $this->hrefMainNew      = 'new_app_menu';
        $this->routerPost       = 'postAppMenu';

        $this->pageTitleMain    = Lang::get($this->langText.'.base_11100');//大標題
        $this->pageTitleList    = Lang::get($this->langText.'.base_11101');//列表
        $this->pageNewTitle     = Lang::get($this->langText.'.base_11102');//新增
        $this->pageEditTitle    = Lang::get($this->langText.'.base_11103');//編輯
    ...
    public function index(Request $request)
    {
        ...
        $out = $js ='';
        $no        = 0;
        //$parentAry = app_menu::getParentSelect();
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = app_menu::where('parent_id',$parent)->orderby('show_order')->get();
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.base_11111')]; //下一層
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                ...
                if(!$lastParent)
                {
                    ...
                    $childbtn     = HtmlLib::btn(SHCSLib::url($this->hrefMainSelect,'','pid='.$id.'&fid='.$parent),Lang::get('sys_btn.btn_31'),4);
                }
                //如果是 本身不是最高權限群組
                if($this->isWirte != 'Y')
                {
    ...
    public function post(Request $request)
    {
        ...
        if($isNew)
        {
            $ret = $this->createAppMenu($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setAppMenu($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'app_menu',$id);
    ...
    public function create()
    {
        ...
        //參數
        $js = $contents = '';
        $menuAry    = app_menu::getParentSelect();
        //view元件參數
        $hrefBack   = $this->hrefMain;
        $btnBack    = $this->pageBackBtn;
