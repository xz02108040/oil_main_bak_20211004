...
class R01022 extends JsonApi
{
    ...
    public function toShow() {
        ...
        $this->be_dept_id       = (isset($jsonObj->be_dept_id))?  $jsonObj->be_dept_id : 0;  //
        if(is_string($this->b_factory_a_id) && $this->b_factory_a_id != "0") $this->b_factory_a_id = explode(',',$this->b_factory_a_id);
        if(is_string($this->be_dept_id) && $this->be_dept_id != "0") $this->be_dept_id = explode(',',$this->be_dept_id);
        //授權來源
        if($token == $this->token)
        {
            $this->errCode = '';
    ...
    public function genReplyAry()
    {
        ...
        if($this->reply == 'Y')
        {
            $today = date('Y-m-d');
            //回傳內容
            $reptAry   =  $this->genPermitReptToday($this->b_factory_id,$this->b_factory_a_id,$this->be_dept_id);
            $ret['data']        =  $reptAry;
            $ret['total_men']   = $this->getDoorMenInOutTodayFactoryData($today,$this->b_factory_id,0,0,1,1,'Y');
            $ret['total_car']   = $this->getDoorCarInOutTodayFactoryData($today,$this->b_factory_id,0,0,1,1,'Y');
            $ret['total_work']  = view_wp_work::getTodayCount($this->b_factory_id,$this->b_factory_a_id,$this->be_dept_id);
