...
class e_license_type extends Model
{
    /**
     * 使用者Table:
     */
    protected $table = 'e_license_type';
    /**
     * Table Index:
     */
    ...
    {
        if(!$id) return 0;
        $data  = e_license_type::where('id',$id);
        return $data->count();
    }
    ...
    {
        $ret  = [];
        $data = e_license_type::select('id','name')->where('isClose','N');
