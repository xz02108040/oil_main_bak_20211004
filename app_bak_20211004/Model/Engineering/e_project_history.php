...
class e_project_history extends Model
{
    /**
     * 使用者Table:
     */
    protected $table = 'e_project_history';
    /**
     * Table Index:
     */
    ...
    protected function isExist($project_id,$course_id,$extid = 0)
    {
        ...
        $data  = e_project_history::where('e_project_c.e_project_id',$project_id)->
    ...
    {
        $ret  = [];
        $data = e_project_history::where('e_project_c.e_project_id',$project_id)->where('e_project_c.isClose','N');
        if($isDoorRule)
        {
