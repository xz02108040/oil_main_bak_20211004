...
class e_violation_law extends Model
{
    /**
     * 使用者Table:
     */
    protected $table = 'e_violation_law';
    /**
     * Table Index:
     */
    ...
    {
        if(!$id) return 0;
        $data  = e_violation_law::where('id',$id);
        return $data->count();
    }
    ...
    {
        $ret  = [];
        $data = e_violation_law::select('id','name')->where('isClose','N');
