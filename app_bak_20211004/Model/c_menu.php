...
class c_menu extends Model
{
    ...
    protected  function getNextLevelMenu($id,$isSelect = 0)
    {
        if(!$id || $id < 0) $id = 1;
        $ret = array();
        $data =  c_menu::where('parent_id',$id)->get();
        if(count($data))
        {
            if($isSelect)
            {
    ...
    protected  function getBelongMenu($id)
    {
        if(!$id) return false;
        $ret = array($id);
        $parent_id = c_menu::find($id)->parent_id;
    ...
    //取得 下拉選擇全部
    protected  function getSelect($isFirst = 1,$isTag = 1)
    {
        $ret  = [];
        $menu = c_menu::orderby('parent_id')->orderby('show_order')->select('id','name','parent_id');
        $menu = $menu->get();
