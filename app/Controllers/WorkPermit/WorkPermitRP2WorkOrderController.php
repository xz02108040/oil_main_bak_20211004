...
class WorkPermitRP2WorkOrderController extends Controller
{
    ...
    */
    ...
    public function index(Request $request)
    {
        ...
        $wpSearch   = [0,$pid,'','',0];
        $storeSearch= [0,0,0];
        $depSearch  = [$this->be_dept_id,0,0,0,0,0];
        $dateSearch = [$today,'','Y'];;
                ...
                else
                {
                    ...
                    if(count($value['option']))
                    {
                        foreach ($value['option'] as $key => $val1)
                        {
                            ...
                            elseif(in_array($wp_option_type,[2,4,3,10,13,14,18]))
                            {
                            ...
                            if($val['wp_option_type'] == 1)
                            {
                                if($val['ans_type'] == 4)
                                {
                                    ...
                                    if($topic_a_id == 120)
                                    {
                                        ...
                                        //結束時間
                                        $timeAry = SHCSLib::genTimeAry('23:59',10);
                                        $html .= $form->textSelect('topic['.$topic_a_id.']',$timeAry,$val['ans_value'],$val['name'],'timeclock').'<br/>';
                                    }
                                }
                                elseif(in_array($topic_a_id,[122,123,124,14,42,165,167]) && count($ans_select))
                                {
                                    $selectAry = [];
...
//                                        dd($val1);
        ...
        if($isReady == 'Y')
        {
            //$submitDiv .= $form->submit(Lang::get('sys_btn.btn_46'),'1','agreeReady').'&nbsp;';
        }
//        if($isEditWorkTime == 'Y')
//        {
...
//        dd([$allTopic,$topicAry]);
        ...
        if($isNew)
        {
            ...
            //啟動階段
            elseif($request->has('agreeReady'))
            {
                //程序ＩＤ
//                $this->process_id   = wp_permit_process::isStatusExist($permit_id,1,1,1);
//
//                $upAry = [];
//                $upAry['aproc']                 = 'P'; //啟動
//                $upAry['wp_permit_process_id']  = $this->process_id;
//                $upAry['check_stime']           = $now;
////                dd([$upAry,$list_id]);
//                if($this->setWorkPermitWorkOrderList($list_id,$upAry,$this->b_cust_id))
//                {
//                    $ret      = 1;
//                    $suc_msg  = 'base_10128';
//                    //設定 自動跳到下一個階段
//                    $backHref = SHCSLib::url($this->hrefMainDetail,$id);
//                    Session::put($this->hrefMain.'.search.aproc','P');
//                    //推播： 工作許可證啟動通知->承攬商：工地負責人/安衛人員
//                    $this->pushToSupplyPermitWorkReady($id,$now);
//                } else {
//                    $ret     = 0;
//                    $err_msg = 'base_10129';
//                }
                ...
                //修改
                $ret = $this->setApiWorkPermitTopicRecord($myTarget,$id,$list_id,$work_process_id,$process_id,(object)$ans,'N','N',$this->b_cust_id,$reject_memo);
                $suc_msg = 'base_10131';
                $err_msg = 'base_10132';
            }
        }
