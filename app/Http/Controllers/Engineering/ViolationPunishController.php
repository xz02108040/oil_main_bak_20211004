...
use App\Http\Traits\Engineering\ViolationPunishTrait;
...
class ViolationPunishController extends Controller
{
    use ViolationPunishTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | ViolationPunishController
    |--------------------------------------------------------------------------
    |
    | 違規罰則 維護
    |
    */
    ...
    public function __construct(Request $request)
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'eviolationpunish';
        $this->langText         = 'sys_engineering';

        $this->hrefMainDetail   = 'eviolationpunish/';
        $this->hrefMainNew      = 'new_eviolationpunish';
        $this->routerPost       = 'postEviolationpunish';
...
//        $btnBack  = $this->pageBackBtn;
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiViolationPunishList();
    ...
    public function show(Request $request,$urlid)
    {
        ...
        $form->add('nameT3', $html,Lang::get($this->langText.'.engineering_32'));
        //停用
    ...
    public function post(Request $request)
    {
        ...
        if($isNew)
        {
            $ret = $this->createViolationPunish($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setViolationPunish($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'e_violation_punish',$id);
