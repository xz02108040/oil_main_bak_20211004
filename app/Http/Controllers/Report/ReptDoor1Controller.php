...
class ReptDoor1Controller extends Controller
{
    ...
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'rept_door_l1';
        $this->langText         = 'sys_rept';

        $this->pageTitleMain    = Lang::get($this->langText.'.title18');//大標題
        $this->pageTitleList    = Lang::get($this->langText.'.list18');//列表
    ...
    public function index(Request $request)
    {
        ...
        if(($bid || $aid))
        {
            list($listAryCnt,$listAry) = $this->getDoorTodayRept([1,0,$aid,0,$bid]);
            if($request->has('showtest'))
            {
                dd([1,0,$aid,0,$bid],$listAry);
            }
        }
        //-------------------------------------------//
        //  View -> Gird
        ...
        $html = $form->select('aid',$supplyAry,$aid,3,Lang::get($this->langText.'.rept_203'));
        $html.= $form->text('bid',$bid,3,Lang::get($this->langText.'.rept_10'));
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.rept_315')]; //
        $heads[] = ['title'=>Lang::get($this->langText.'.rept_306')]; //
        ...
        if($listAryCnt)
        {
            ...
            foreach($listAry as $value)
            {
                $no++;
                $id              = $value->id;
                $rept1           = $value->store.'/'.$value->local;
                $rept2           = $value->unit_name;
                $rept3           = $value->project;
                $rept4           = $value->name;
                $rept5           = $value->door_type_name;
                $rept6           = $value->door_stamp;
                $rept7           = Lang::get($this->langText.'.rept_312',['name1'=>$value->work_no]);
