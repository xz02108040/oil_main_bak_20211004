...
class DoorCarWhiteOrderController extends Controller
{
    use ReptDoorOrderTrait,SessTraits,DoorTrait;
    /*
    |--------------------------------------------------------------------------
    | DoorCarWhiteOrderController
    |--------------------------------------------------------------------------
    |
    | 報表: 門禁白名單報表(車輛)
    |
    */
    ...
    public function index(Request $request)
    {
        ...
        $lnspectionAry  = SHCSLib::getCode('CAR_YEAR_LNSPECTION2');
        //view元件參數
        ...
        $listAry = $this->getDoorCarOrderList();
        ...
        $form->addHtml(HtmlLib::Color(Lang::get($this->langText.'.rept_100007'),'red',1));
        $form->addHr();
        //輸出
        $out .= $form->output(1);
        //table
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.rept_240')]; //柴油車
        $heads[] = ['title'=>Lang::get($this->langText.'.rept_241')]; //驗排氣
        $heads[] = ['title'=>Lang::get($this->langText.'.rept_242')]; //下次驗氣日
        ...
        if($listAmt)
        {
            foreach($listAry as $value)
            {
                ...
                $name4        = isset($oilkindAry[$value->oil_kind])? $oilkindAry[$value->oil_kind] : ''; //
                $name5        = ($value->oil_kind == 2)? isset($lnspectionAry[$value->isInspectionExhaust])? $lnspectionAry[$value->isInspectionExhaust] : '' : ''; //
                $name6        = ($value->oil_kind == 2)? $value->last_exhaust_inspection_date2 : ''; //
