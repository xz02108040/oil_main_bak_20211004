...
class Rept18Controller extends Controller
{
    use SessTraits;
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'report_18';

        $this->pageTitleMain    = '承攬商違規排行板-每月';//大標題
...
//        if(!$aid || !$bid)
        {
            ...
            $SQL = "SELECT TOP ".$aid." --設定排名資料
					C.id AS 編號, C.name 承攬商,COUNT(*) 筆數
					...
					WHERE $query
					--AND DATEPART(MONTH,A.apply_date)=1      --(月份)
            ...
            if($request->has('download'))
            {
                ...
                return Excel::download(new ExcelExport(), '承攬商違規排行板_'.date('Ymdhis').'.xlsx');
            }
        }
        //-------------------------------------------//
        //  View -> Gird
