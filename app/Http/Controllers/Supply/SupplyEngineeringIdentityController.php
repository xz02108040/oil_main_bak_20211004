...
class SupplyEngineeringIdentityController extends Controller
{
    use SupplyEngineeringIdentityTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | SupplyEngineeringIdentityController
    |--------------------------------------------------------------------------
    |
    | 工程身分 維護
    |
    */
    ...
    public function __construct(Request $request)
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'engineeringidentity';
        $this->hrefLicense      = 'engineeringidentitylicense';
        $this->langText         = 'sys_supply';

        $this->hrefMainDetail   = 'engineeringidentity/';
        $this->hrefMainNew      = 'new_engineeringidentity';
        $this->routerPost       = 'postEngineeringidentity';
...
//        $btnBack  = $this->pageBackBtn;
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiSupplyEngineeringIdentityList();
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.supply_81')]; //審查分類
        $heads[] = ['title'=>Lang::get($this->langText.'.supply_11')]; //排序
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                ...
                $btn2         = HtmlLib::btn(SHCSLib::url($this->hrefLicense,'','iid='.SHCSLib::encode($id)),Lang::get('sys_btn.btn_30'),4); //按鈕
    ...
    public function show(Request $request,$urlid)
    {
        ...
        //不可以修改的分類
        $kindAry  = SHCSLib::getCode('IDENTITY_CHARGE_KIND',1);
        $limitStr = sys_param::getParam('SUPPLY_IDENTITY_TYPE_EDIT_LIMIT','');
        $limitAry = explode(',',$limitStr);
        //view元件參數
        ...
        if(!isset($getData->id))
        {
            ...
            $A3         = $getData->charge_kind; //
            $A4         = $getData->charge_kind_name; //

            $isEdit     = in_array($id,$limitAry)? 0 : 1;
        ...
        //審查分類
        $html = $form->select('charge_kind',$kindAry,$A3);
        $form->add('nameT1', $html,Lang::get($this->langText.'.supply_81'),1);
        //排序
        $html = $form->text('show_order',$A2);
        $form->add('nameT3', $html,Lang::get($this->langText.'.supply_11'));
        //停用
        if($isEdit)
        {
            $html = $form->checkbox('isClose','Y',$A99);
        } else {
            $html = HtmlLib::Color(Lang::get($this->langText.'.supply_1017'),'red');
        }
    ...
    public function post(Request $request)
    {
        ...
        //名稱重複
        elseif(b_supply_engineering_identity::isExist($request->name,SHCSLib::decode($request->id)))
        {
            return \Redirect::back()
        ...
        $upAry['charge_kind']       = is_numeric($request->charge_kind) ? $request->charge_kind : 1;
        ...
        if($isNew)
        {
            $ret = $this->createSupplyEngineeringIdentity($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setSupplyEngineeringIdentity($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
    ...
    public function create()
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.supply_51'),1);
        //審查分類
        $html = $form->select('charge_kind',$kindAry,1);
        $form->add('nameT1', $html,Lang::get($this->langText.'.supply_81'),1);
        //排序
        $html  = $form->text('show_order');
        $form->add('nameT2', $html,Lang::get($this->langText.'.supply_11'));
