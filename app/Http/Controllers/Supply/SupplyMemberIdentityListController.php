...
class SupplyMemberIdentityListController extends Controller
{
    use SupplyMemberIdentityTrait,SupplyMemberLicenseTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | SupplyMemberIdentityListController
    |--------------------------------------------------------------------------
    |
    | 承攬商成員_擁有承攬身分
    |
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = 'contractormemberlist';
        $this->hrefMain         = 'contractormemberidentitylist';
        $this->langText         = 'sys_supply';

        $this->hrefMainDetail   = 'contractormemberidentitylist/';
        $this->hrefMainDetail2  = 'contractormemberlicenselist/';
    ...
    public function index(Request $request)
    {
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                ...
                //按鈕
                $btn          = HtmlLib::btn(SHCSLib::url($this->hrefMainDetail,$id,$param1),Lang::get('sys_btn.btn_30'),1); //按鈕
    ...
    public function show(Request $request,$urlid)
    {
        ...
        if(!isset($getData->id))
        {
            ...
            $isOver     = 0;//(strtotime($A2) >= strtotime(date('Y-m-d')))? 0 : 1;
...
//
//                        }
                        if($edate_type == 'edate' && strtotime($edate) < strtotime(date('Y-m-d')))
                        {
                            //日期不可小於今日
                            return \Redirect::back()
                    ...
                    elseif(!CheckLib::isDate($val['edate']))
                    {
                        ...
                        if($file1)
                        {
                            ...
                            //dd([$File,$extension]);
                            //[錯誤]格式錯誤
                            if(!in_array(strtoupper($extension),['JPG','PNG','GIF','JPEG'])){
                                return \Redirect::back()
...
//        dd($upAry);
    ...
    public function create(Request $request,$urlid)
    {
        ...
        elseif(!$request->license_id && count($linceseAry))
        {
            ...
            {
                list($show_name1,$show_name2,$edate_type) = e_license::getShowList($lid);
                $form->addHr();
                //證件
                ...
                $form->add('nameT3', $html,Lang::get($this->langText.'.supply_34'));
                //證明1－檔案2
