...
use App\Http\Traits\WorkPermit\WorkPermitShiftTrait;
...
class WorkPermitShiftController extends Controller
{
    use WorkPermitShiftTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | WorkPermitShiftController
    |--------------------------------------------------------------------------
    |
    | 工作許可證 班別 維護
    |
    */
    ...
    public function __construct(Request $request)
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'workpermitshift';
        $this->langText         = 'sys_workpermit';

        $this->hrefMainDetail   = 'workpermitshift/';
        $this->hrefMainNew      = 'new_workpermitshift';
        $this->routerPost       = 'postWorkpermitshift';

        $this->pageTitleMain    = Lang::get($this->langText.'.title30');//大標題
        $this->pageTitleList    = Lang::get($this->langText.'.list30');//標題列表
        $this->pageNewTitle     = Lang::get($this->langText.'.new30');//新增
        $this->pageEditTitle    = Lang::get($this->langText.'.edit30');//編輯
    ...
    public function index(Request $request)
    {
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiWorkPermitListShift();
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.permit_53')]; //開始時間
        $heads[] = ['title'=>Lang::get($this->langText.'.permit_54')]; //結束時間
        $heads[] = ['title'=>Lang::get($this->langText.'.permit_55')]; //是否過夜
        ...
        if(count($listAry))
        {
            foreach($listAry as $value)
            {
                ...
                $tBody[] = ['0'=>[ 'name'=> $no,'b'=>1,'style'=>'width:5%;'],
                            ...
                            '2'=>[ 'name'=> $name2],
                            '3'=>[ 'name'=> $name3],
                            '4'=>[ 'name'=> $name4,'label'=>$isOptionColor],
    ...
    public function show(Request $request,$urlid)
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_52'),1);
        //開始時間
        $html = $form->text('stime',$A2);
        $form->add('nameT3', $html,Lang::get($this->langText.'.permit_53'));
        //結束時間
        $html = $form->text('etime',$A3);
        $form->add('nameT3', $html,Lang::get($this->langText.'.permit_54'));
        //是否過夜
        $html = $form->checkbox('isAcrossNight','Y',$A4);
        $form->add('isCloseT',$html,Lang::get($this->langText.'.permit_55'));
        //停用
    ...
    public function post(Request $request)
    {
        ...
        elseif ( !$request->stime || !$request->etime)
        {
            return \Redirect::back()
                ->withErrors(Lang::get($this->langText.'.permit_11007'))
                ->withInput();
        }
        elseif ( wp_permit_shift::isNameExist($request->name,$id))
        {
            return \Redirect::back()
                ->withErrors(Lang::get($this->langText.'.permit_11008'))
                ->withInput();
        }
        else {
        ...
        $upAry['stime']             = $request->stime ? $request->stime : 0;
        $upAry['etime']             = $request->etime ? $request->etime : 0;
        $upAry['isAcrossNight']     = $request->isAcrossNight ? $request->isAcrossNight : 'N';
        ...
        if($isNew)
        {
            $ret = $this->createWorkPermitShift($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setWorkPermitShift($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
            ...
            if($ret === -1)
            {
                ...
                //動作紀錄
                LogLib::putLogAction($this->b_cust_id,$menu,$ip,$action,'wp_permit_shift',$id);
    ...
    public function create()
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_52'),1);
        //開始時間
        $html = $form->text('stime');
        $form->add('nameT3', $html,Lang::get($this->langText.'.permit_53'));
        //結束時間
        $html = $form->text('etime');
        $form->add('nameT3', $html,Lang::get($this->langText.'.permit_54'));
        //是否過夜
        $html = $form->checkbox('isAcrossNight','Y');
        $form->add('isCloseT',$html,Lang::get($this->langText.'.permit_55'));
