...
class WorkPermitWorkItemDangerController extends Controller
{
    use WorkPermitWorkItemDangerTrait,SessTraits;
    /*
    |--------------------------------------------------------------------------
    | WorkPermitWorkItemDangerController
    |--------------------------------------------------------------------------
    |
    | 工作許可證 工作項目之危險告知維護
    |
    */
    ...
    public function __construct()
    {
        ...
        //路由
        $this->hrefHome         = '/';
        $this->hrefMain         = 'workpermitworkitemdanger';
        $this->hrefKind         = 'workpermitkind';
        $this->hrefItem         = 'workpermitworkitem';
        $this->langText         = 'sys_workpermit';

        $this->hrefMainDetail   = 'workpermitworkitemdanger/';
        $this->hrefMainNew      = 'new_workpermitworkitemdanger/';
        $this->routerPost       = 'postWorkpermitworkitemdanger';
    ...
    public function index(Request $request)
    {
        ...
        //-------------------------------------------//
        //抓取資料
        $listAry = $this->getApiWorkPermitWorkItemDangerList($tid);
        ...
        $heads[] = ['title'=>Lang::get($this->langText.'.permit_45')]; //危險等級
    ...
    public function show(Request $request,$urlid)
    {
        ...
        if(!isset($getData->id))
        {
            ...
            $A5         = $getData->wp_permit_danger_id; //
        ...
        //種類
        $html = $form->select('danger_id',$dangerAry,$A5);
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_45'),1);
        //排序
    ...
    {
        //資料不齊全
        if( !$request->has('agreeY') || !$request->id || !$request->danger_id  || !$request->workitem_id )
        {
        ...
        //相同數值
        elseif(wp_permit_workitem_a::isExist($request->workitem_id,$request->danger_id,SHCSLib::decode($request->id)))
        {
            return \Redirect::back()
        ...
        $upAry['wp_permit_danger_id']   = $request->danger_id;
        ...
        if($isNew)
        {
            $ret = $this->createWorkPermitWorkItemDanger($upAry,$this->b_cust_id);
            $id  = $ret;
        } else {
            //修改
            $ret = $this->setWorkPermitWorkItemDanger($id,$upAry,$this->b_cust_id);
        }
        //2-1. 更新成功
        if($ret)
        {
    ...
    public function create(Request $request,$urlid)
    {
        ...
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_108'),1);
        //種類
        $html = $form->select('danger_id',$dangerAry);
        $form->add('nameT1', $html,Lang::get($this->langText.'.permit_45'),1);
        //排序
