...
//            return $pdf->download($permit_no.'.pdf');
    ...
    public function print_main1()
    {
        ...
        //車牌
        $car_no             = $this->workData->b_car_memo; // rept_doorinout_car_t::getTodayCar($this->workData->b_factory_id,$this->workData->b_supply_id,2);
        //
        $work_place         = b_factory_a::getName($this->workData->b_factory_a_id) . ' > ' . b_factory_b::getName($this->workData->b_factory_b_id).'('.$this->workData->b_factory_memo.')';
        //
...
//        dd($chk_supply_topic34);
    ...
    public function print_main3(){
        ...
        $chk_supply_topic109   = isset($chk_supply_topic99[253])?    $chk_supply_topic99[253] : '';//附註
        //承攬商簽名
        ...
        $chk_emp_topic109   = isset($chk_emp_topic99[256])?     $chk_emp_topic99[256]   : '';//附註
        //檢點者
        ...
        if ($chk_emp_delay) { //新增判斷，避免沒有延時資料導致異常
            ...
            $chk_delay_charge3_topic     = ($chk_emp_delay['charge_user3'] != '0') ? $this->checkAry['Y'] : $this->checkAry['N'];
            // $charge_name1 = ($chk_emp_delay['img1']) ?  $this->genImgHtml($chk_emp_delay['img1'], '') : $chk_emp_delay['charge_name1'];
            // $charge_name2 = ($chk_emp_delay['img2']) ?  $this->genImgHtml($chk_emp_delay['img2'], '') : $chk_emp_delay['charge_name2'];
            $chk_delay_charge1 = ($chk_emp_delay['charge_user1'] != '0') ? $chk_emp_delay['charge_name1'] : '';
            $chk_delay_charge2 = ($chk_emp_delay['charge_user2'] != '0') ? $chk_emp_delay['charge_name2'] : '';
            $chk_delay_charge3 = ($chk_emp_delay['charge_user3'] != '0') ? $chk_emp_delay['charge_name3'] : '';
        } else {
            $eta_etime1_a =  '';
            $eta_etime1_b =  '';
            $eta_etime2_a = '';
            $eta_etime2_b = '';
            $chk_delay_charge1_topic     = '';
            $chk_delay_charge2_topic     = '';
            $chk_delay_charge3_topic     = '';
            $chk_delay_charge1 = '';
            $chk_delay_charge2 = '';
            $chk_delay_charge3 = '';
        }
        ...
        if($this->permit_danger == 'A')
        {
            ...
            $chk_emp_topic37 = $chk_emp_topic37h . '<br/>監工：'.$chk_emp_topic37a.'<br>承商：'.$chk_emp_topic37b.'<br/>轄區：'.$chk_emp_topic37c;
...
//        dd($sign_url14,$sign_url15,$sign_url16,$sign_url17,$sign_url18,$sign_url19);
        ...
        $chk_emp_topic51_date    = ($chk_emp_topic51)? SHCSLib::addDay(1, $this->sdate) : '';
        ...
        if($chk_emp_topic51)
        {
            ...
            //夜班時段客製化顯示
            if ($this->workData->wp_permit_shift_id == '2') {
                if ($topic51Ary[0] > 12)  $chk_emp_topic50 = '下午 ' . $chk_emp_topic50 . ' 至 ' . $chk_emp_topic51;
                if ($topic51Ary[0] <= 12) $chk_emp_topic50 = '下午 ' . $chk_emp_topic50 . ' 至 ' . $chk_emp_topic51 . ' (' . $chk_emp_topic51_date . ')';
                $chk_emp_topic51 = '';
            } else {
                if ($topic51Ary[0] <= 12) $chk_emp_topic51 = '上午 ' . $chk_emp_topic51 . ' 至 12:00';
                //白天下午班只顯示下午時段，其他上下午兩個時段則正常顯示
                if ($topic50Ary[0] > 12 && $topic51Ary[0] > 12) {
                    $chk_emp_topic50 = '下午 ' . $chk_emp_topic50 . ' 至 ' . $chk_emp_topic51;
                    $chk_emp_topic51 = '';
                } else {
        ...
        $showAry['chk_supply_topic'][109] = $chk_supply_topic109;
        //轄區氣體偵測
        ...
        $showAry['chk_emp_topic'][109] = $chk_emp_topic109; // 附註
        ...
        $showAry['chk_delay_charge3_topic'] = $chk_delay_charge3_topic; //施工
        $showAry['chk_delay_charge3'] = $chk_delay_charge3; //施工
