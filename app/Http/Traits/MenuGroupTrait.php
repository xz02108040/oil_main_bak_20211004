...
trait MenuGroupTrait
{
    ...
    public function createMenuGroup($data,$mod_user = 1)
    {
        ...
        $b_menu_group = new b_menu_group();
        $b_menu_group->name       = $data->name;
        $b_menu_group->kind       = in_array($data->kind,['A','B'])? $data->kind : 'A';
    ...
    public function setMenuGroup($id,$data,$mod_user = 1)
    {
        ...
        $b_menu_group = b_menu_group::find($id);
        //名稱
        ...
        //名稱
        if(isset($data->kind) && $data->kind && $data->kind !== $b_menu_group->kind)
        {
            $isUp++;
            $b_menu_group->kind = $data->kind;
        }
        //排序
     ...
     * @return array
     */
    public function getApiMenuGroupList($kind = 'X')
    {
        $ret = array();
        //取第一層
        $data = b_menu_group::where('kind',$kind)->orderby('isClose')->orderby('show_order')->get();
