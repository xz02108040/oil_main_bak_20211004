...
//                dd($b_supply_member_l_id2,$tmp);
    ...
    public function createEngineeringMemberIdentity($data,$mod_user = 1)
    {
        ...
        if($ret)
        {
            // 項次346，專案角色由系統判斷，第一優先為工安工負，其他為施工人員
            $myIdentity = e_project_license::isWhoIdenttity($INS->e_project_id,$INS->b_cust_id);
            ...
            if( in_array($myIdentity,[$identity_A,$identity_B,$identity_C]))
            {
                //更新中油標籤
                $cpc_tag  = ($myIdentity == $identity_A)? 'A' : 'B';
                $job_kind = ($myIdentity == $identity_A)? 2 : 3;
                $cpc_tag  = ($myIdentity == $identity_C)? 'C' : $cpc_tag;
                $job_kind = ($myIdentity == $identity_C)? 1 : $job_kind;
