...
//        }
     ...
     * @return array
     */
    public function getApiMenuList()
    {
        $ret = array();
        //取第一層
        $data = c_menu::where('isClose','N')->orderby('show_order')->get();
     ...
     * @return array
     */
    public function getApiMenu($b_menu_group_id)
    {
        $ret = array();
        $authAry     = $this->getApiMenuAuthList($b_menu_group_id);
        //取第一層
        $data = c_menu::where('isClose','N')->where('isShow','Y')->orderby('show_order')->get();
            ...
            if(count($menuAry))
            {
                foreach ($menuAry[0] as $v)
                {
                    ...
                    if(isset($menuAry[$pid]))
                    {
                        ...
                        $tmp['submenu']       = $this->getLoopMenu($menuAry,$pid);
                    }
                    $ret[] = $tmp;
                }
            }
     ...
     * @return array
     */
    public function getLoopMenu($menuAry,$pid = 0)
    {
        $ret = array();
        if(!$pid || !isset($menuAry[$pid])) return $ret;
