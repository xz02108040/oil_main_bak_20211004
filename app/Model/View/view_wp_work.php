...
class view_wp_work extends Model
{
    /**
     * 使用者Table: 列出 目前進行中之工單
     */
    protected $table = 'View_wp_work';
    /**
     * Table Index:
     */
    ...
    {
        if(!$id) return 0;
        $data = view_wp_work::where('name',$id);
        if($extid)
        {
    ...
    {
        if(!$wid) return 0;
        $data = view_wp_work::where('id',$wid)->whereIn('aproc',['P','K','R','O'])->where('isClose','N');
        $data = $data->select('view_wp_work.id')->first();
        return isset($data->id)? $data->id : 0;
    }
    protected function isAdSafeUser($wid,$uid)
    {
        $data = view_wp_work::where('id',$wid)->Where('view_wp_work.supply_safer', $uid);
        $data = $data->select('view_wp_work.id')->first();
        return isset($data->id)? $data->id : 0;
    }
    ...
    protected  function getNo($id,$showType = 1)
    {
        ...
        if($showType == 2)
        {
            $data = view_wp_work::join('b_factory_d as d','d.id','=','view_wp_work.b_factory_d_id')->
            where('view_wp_work.id',$id)->select('permit_no','d.name')->first();
            $ret  = (isset($data->permit_no) && $data->permit_no)? $data->permit_no.'('.$data->name.')' : '';
        } else {
            $data = view_wp_work::where('id',$id)->select('permit_no')->first();
    ...
    {
        if(!$id) return 0;
        $data = view_wp_work::where('id',$id)->select('b_factory_id')->first();
    ...
    protected  function getLocalGPS($id)
    {
        $ret = ['',0,0];
        if(!$id) return $ret;
        $data = view_wp_work::where('id',$id)->select('b_factory_b_id')->first();
    ...
    {
        if(!$id) return 0;
        $data = view_wp_work::where('id',$id)->where('isClose','N')->select('id','apply_user')->first();
    ...
    protected  function getDept($id,$type = 1)
    {
        $ret = 0;
        if(!$id) return '';
        $data = view_wp_work::where('id',$id)->where('isClose','N')->first();
        //轄區部門上一層
        if($type == 5)
        {
            if(isset($data->be_dept_id1) && $data->be_dept_id1 > 0)
            {
                $ret = be_dept::getParantDept($data->be_dept_id1);
            }
        }
        elseif($type == 4)
        {
    ...
    {
        if(!$id) return 0;
        $data = view_wp_work::where('id',$id)->where('isClose','N')->select('id','wp_permit_id')->first();
    ...
    {
        if(!$id) return '';
        $data = view_wp_work::where('id',$id)->where('isClose','N')->select('id','aproc')->first();
    ...
    {
        if(!$id) return 0;
        $data = view_wp_work::where('id',$id)->where('isClose','N')->select('id','proejct_charge')->first();
    ...
    {
        if(!$id) return [0,''];
        $data = view_wp_work::where('id',$id)->where('isClose','N')->select('id','b_factory_d_id')->first();
...
//        $data = view_wp_work::where('e_project_id',$project_id)->where('sdate',date('Y-m-d'))->where('isClose','N')->
...
//    }
    ...
    protected  function getContactType($work_id)
    {
        ...
        $data = view_wp_work::find($work_id);
        if(isset($data->id))
        {
    ...
    protected function isWorkerExist($project,$store,$uid)
    {
        ...
        $data  = view_wp_work::join('wp_work_worker as w','w.wp_work_id','=','view_wp_work.id')->
        where('w.user_id',$uid)->where('w.isClose','N')->where('view_wp_work.b_factory_id',$store)->
        where('view_wp_work.e_project_id',$project)->where('view_wp_work.sdate',date('Y-m-d'))->where('view_wp_work.isClose','N')->
        whereNotIn('view_wp_work.aproc',['A','B'])->select('w.aproc')->first();
    ...
    //取得 未啟動之工單
    protected  function getNotReadyAry($project_id,$aprocAry = ['W','C','F'])
    {
        $ret = [0];
        if(!$project_id) return $ret;
        $data = view_wp_work::where('e_project_id',$project_id)->where('isClose','N')->whereIn('aproc',$aprocAry)->select('id');
    ...
    {
        $work_id = $isRoot = $isIn = $identity = $aproc = $danger = '';
        $isAdd      = $isLevel = 0;
        $errCodeNo  = 3;
...
//        $RootAry    = ($door_type == 2)? [$identity_B] : [$identity_A,$identity_B];
        ...
        if(!$work_id)
        {
            ...
            $data  = view_wp_work::join('wp_work_worker as w','w.wp_work_id','=','view_wp_work.id')->
            where('w.user_id',$uid)->where('w.isClose','N')->where('view_wp_work.b_factory_id',$store)->
            where('view_wp_work.b_factory_d_id',$door)->
            where('view_wp_work.e_project_id',$project)->
            where('view_wp_work.isClose','N');
            //2-2. 審查通過/已啟動/施工中/申請收工 + 已停工/已收工
            $data  = $data->whereIn('view_wp_work.aproc',['P','W','R','O']);
            $data  = $data->whereNotIn('w.aproc',['A','C','L']);
            $data  = $data->whereIn('w.engineering_identity_id',[$identity_A,$identity_B]);
            $data  = $data->select('view_wp_work.id','w.engineering_identity_id','view_wp_work.aproc','view_wp_work.wp_permit_danger','w.aproc as worker_aproc');
            ...
            if($data->count())
            {
                ...
                foreach ($data as $val)
                {
                    ...
                    $isRoot         = in_array($identity,$RootAry)? 1 : 0;
                    //dd('A',$uid,$val->id,$aproc,$worker_aproc,$identity);
                    $tmp = [];
                    $tmp['work_id']     = $val->id;
                    $tmp['identity']    = $identity;
                    $tmp['isRoot']      = in_array($identity,$RootAry)? 1 : 0;
                    $tmp['isAdd']       = (in_array($aproc,['W','P','R']) && $worker_aproc == 'P')? 1 : 0;
                    $tmp['isRoot']      = $isRoot;
                    if($door_type == 2)
                    {
                        $tmp['isIn']        = 1;
                        ...
                        $isNotRootOut       = (in_array($aproc,['C','F']) && !$isRoot)? 1 : 0;
                        $tmp['isIn']        = ($worker_aproc == 'L' || $isNotRootOut)? 0 : 1;
                        $tmp['errCode']     = ($worker_aproc == 'L')? 'base_30161' : ($isNotRootOut ? 'base_30160' : '');
                        $tmp['errCodeNo']   = ($worker_aproc == 'L')? 6 : ($isNotRootOut ? 7 : 0);
                    }
                ...
                where('view_wp_work.isClose','N');
                //2-4. 審查通過/已啟動/施工中/申請收工 + 已停工/已收工
                $data  = $data->whereIn('view_wp_work.aproc',['F','C']);
                $data  = $data->whereNotIn('w.aproc',['A','C','L']);
                $data  = $data->whereIn('w.engineering_identity_id',[$identity_A,$identity_B]);
                $data  = $data->select('view_wp_work.id','w.engineering_identity_id','view_wp_work.aproc','view_wp_work.wp_permit_danger','w.aproc as worker_aproc');
                ...
                if($data->count())
                {
                    ...
                    foreach ($data as $val)
                    {
                        ...
                        $danger         = $val->wp_permit_danger;
                        $isRoot         = in_array($identity,$RootAry)? 1 : 0;
//                        dd('B',$uid,$val->id,$aproc,$worker_aproc,$identity);
                        $tmp = [];
                        $tmp['work_id']     = $val->id;
                        $tmp['identity']    = $identity;
                        $tmp['isRoot']      = in_array($identity,$RootAry)? 1 : 0;
    ...
    /**
     * 今日進場是否存在 補人<針對自己>的工單
     * @param $project
     * @param $store
     * @param $door
     * @param $uid
     * @param $isAddWorker 是否為補人
     * @return int|mixed
     */
    protected  function getTodayWork2($project,$store,$uid,$isAddWorker = false, $isLock = false)
    {
        $allowWorkOrderAry = ['O'=>[],'R'=>[],'P'=>[],'W'=>[],'F'=>[],'C'=>[]];
        $work_id = 0;
        //2-1. [工安＆工負]先找到今日是否可以進場的工作許可證
        ...
        where('view_wp_work.isClose','N');
        //2-2. 審查通過/已啟動/施工中/申請收工 + 已停工/已收工
        $data  = $data->whereIn('view_wp_work.aproc',['P','W','R']);

        if($isAddWorker) {
            $data  = $data->where('w.apply_type',2)->where('w.aproc','P');
        }

        if($isLock) {
            $data  = $data->where('w.isLock', 'Y')->where('w.aproc','R');
        }

        $data  = $data->select('view_wp_work.id','view_wp_work.aproc');
     ...
     * @return int
     */
    protected function getTodayReadyWork($project,$uid)
    {
        $ret = [];
        if(!$project && !$uid) return [0,$ret];
        //1. 取得目前進出紀錄上面綁定的人
        $work_obj   = rept_doorinout_t::getLockWorkId($uid);
        $work_id    = isset($work_obj->id)? $work_obj->id : 0;
        $work_door  = isset($work_obj->name)? $work_obj->name : '';
        //2.
        $data  = view_wp_work::join('wp_work_worker as w','w.wp_work_id','=','view_wp_work.id')->
        join('b_factory_d as d','d.id','=','view_wp_work.b_factory_d_id')->
        where('w.user_id',$uid)->where('w.isClose','N')->
        where('view_wp_work.e_project_id',$project)->
        whereIn('view_wp_work.aproc',['W','P','K','R','O'])->where('view_wp_work.isClose','N')->
        select('view_wp_work.id','view_wp_work.permit_no','d.name')->groupby('view_wp_work.id','view_wp_work.permit_no','d.name');
        $workAmt = $data->count();
        if($workAmt)
        {
            foreach ($data->get() as $val)
            {
                ...
                $tmp['door']        = $val->name;
                $tmp['isWork']      = ($work_id == $val->id)? 'Y' : 'N';
                $tmp['in_door']     = $work_door;
                $ret[] = $tmp;
            }
        }
    ...
    protected function getTodayFinishWork($project,$uid,$store)
    {
        ...
        where('view_wp_work.b_factory_id',$store)->
        where('view_wp_work.e_project_id',$project)->where('view_wp_work.sdate',date('Y-m-d'))->
        where('view_wp_work.aproc','F')->where('view_wp_work.isClose','N');
    ...
    protected function getTodayCount($store,$local = 0,$dept1 = 0,$extAproc = ['A','B'])
    {
        $data = view_wp_work::where('isClose','N');
        if($store)
        {
        ...
        if(count($extAproc))
        {
            $data = $data->whereNotIn('aproc',$extAproc);
        }
        if($local)
        {
            if(is_array($local))
            {
                $data = $data->whereIn('b_factory_a_id',$local);
            } else {
        ...
        if($dept1)
        {
            if(is_array($dept1))
            {
                $data = $data->whereIn('be_dept_id1',$dept1);
            } else {
                $data = $data->where('be_dept_id1',$dept1);
            }
        }
        return $data->count();
    }
    ...
    protected function getNewPermitNo($permit_head = '')
    {
        ...
        $data = view_wp_work::where('permit_no','like','T-'.$permit_head.'%')->orderby('id','desc');
    ...
    protected  function getWorkAllChargeUser($work_id, $type = 1 )
    {
        ...
        $data   = view_wp_work::where('id',$work_id)->where('isClose','N')->first();
        if(isset($data->id))
        {
            //工作許可證上面的人員
...
//            dd($workerAry,$chargeUserAry,$ret);
    ...
    {
        $ret    = [];
        $data   = view_wp_work::where('e_project_id',$project_id)->select('id','permit_no')->where('isClose','N');
