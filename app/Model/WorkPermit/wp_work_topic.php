...
class wp_work_topic extends Model
{
    /**
     * 使用者Table:
     */
    protected $table = 'wp_work_topic';
    /**
     * Table Index:
     */
    ...
    {
        if(!$id) return 0;
        $data = wp_work_topic::where('name',$id);
        if($extid)
        {
    ...
    {
        $ret    = [];
        $data   = wp_work_topic::select('id','name')->where('isClose','N')->get();
