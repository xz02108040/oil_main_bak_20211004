...
class e_violation_type extends Model
{
    /**
     * 使用者Table:
     */
    protected $table = 'e_violation_type';
    /**
     * Table Index:
     */
    ...
    {
        if(!$id) return 0;
        $data  = e_violation_type::where('id',$id);
        return $data->count();
    }
    ...
    {
        $ret  = [];
        $data = e_violation_type::select('id','name')->where('isClose','N');
